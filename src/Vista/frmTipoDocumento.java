...
import Modelo.Modelo_Tabla_TipDoc;
...
public class frmTipoDocumento extends javax.swing.JInternalFrame {
    ...
    private Modelo_Tabla_TipDoc MTD = new Modelo_Tabla_TipDoc();
    private TipoDocumento objTP = null;
    ...
    private void initComponents() {
        ...
        jPanel3.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Tipos de Documentos Registrados", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 1, 14), new java.awt.Color(255, 255, 255))); // NOI18N
        ...
        jPanel4.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Datos del Tipo Documento", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 1, 14), new java.awt.Color(255, 255, 255))); // NOI18N
    ...
    private void definir_tamaño_columna() {
        ...
        this.tablaDocumento.getTableHeader().setFont(new Font("Tahoma", 1, 14));
    ...
    private void cargarTabla() {
        try {
            ArrayList<TipoDocumento> ltsTipDoc = TipoDocBo.obtenerListaTipDoc(cargo);
            MTD.setListado_TipDoc(ltsTipDoc);
        } catch (Exception e) {
    ...
    private void cargarDatos(TipoDocumento obj) {
    ...
    private void eliminar() {
        if (!(this.cargo.equals("OPB"))) {
            if (this.tablaDocumento.getSelectedRow() != -1) {
                int fila = this.tablaDocumento.getSelectedRow();
                TipoDocumento objTipDoc = MTD.getTipDoc(fila);
                if (objTipDoc.getEstado().equals("Activo")) {
                    int confirm = JOptionPane.showConfirmDialog(this, "¿Está seguro que desea eliminar el Tipo de Documento "
                            + objTipDoc.toString() + " ?", "ELIMINAR TIPO DOCUMENTO", JOptionPane.YES_NO_OPTION);
                    if (confirm == 0) {
                        try {
                            if (TipoDocBo.eliminarTipDoc(objTipDoc, cargo)) {
                                this.cargarTabla();
                                this.limpiar();
                                this.activarControles(false);
                            } else {
                                JOptionPane.showMessageDialog(this, "No se pudo Eliminar", "MENSAJE --> TIPO DOCUMENTO", JOptionPane.ERROR_MESSAGE);
                            }
                        } catch (Exception e) {
                            System.out.println(e.getMessage());
                        }
                    ...
                    JOptionPane.showMessageDialog(this, "El Tipo de Documento selecionado ya se encuentra Inactivo.", "MENSAJE --> TIPO DOCUMENTO", JOptionPane.ERROR_MESSAGE);
                }
            } else {
                JOptionPane.showMessageDialog(this, "Usted no a seleccionado un campo en la tabla.", "MENSAJE --> TIPO DOCUMENTO", JOptionPane.ERROR_MESSAGE);
            }
        } else{
    ...
    private void guardar() {
        if (this.txtNombre.getText().trim().length() != 0) {
            TipoDocumento objTiD = new TipoDocumento(0, this.txtNombre.getText(), this.txtEstado.getText());
            try {
                if (this.objTP == null) {
                    if (TipoDocBo.grabarTipDoc(objTiD, cargo)) {
                        JOptionPane.showMessageDialog(this, "Se Registró Correctamente", "MENSAJE --> TIPO DOCUMENTO", JOptionPane.INFORMATION_MESSAGE);
                    } else {
                        JOptionPane.showMessageDialog(this, "No se pudo Registrar", "MENSAJE --> TIPO DOCUMENTO", JOptionPane.ERROR_MESSAGE);
                    }
                } else {
                    objTiD.setIdTipDoc(this.objTP.getIdTipDoc());
                    if (TipoDocBo.modificarTipDoc(objTiD, cargo)) {
                        JOptionPane.showMessageDialog(this, "Se Modificó Correctamente", "MENSAJE --> TIPO DOCUMENTO", JOptionPane.INFORMATION_MESSAGE);
                    } else {
                        JOptionPane.showMessageDialog(this, "No se pudo Modificar", "MENSAJE --> TIPO DOCUMENTO", JOptionPane.ERROR_MESSAGE);
                    }
                }
            } catch (Exception e) {
                System.out.println(e.getMessage());
            ...
            JOptionPane.showMessageDialog(this, "No a completado el campo Nombre.", "MENSAJE --> TIPO DOCUMENTO", JOptionPane.ERROR_MESSAGE);
        }
    }

    private void buscar() {
        String buscar = this.txtBuscar.getText().trim();
        ArrayList<TipoDocumento> ltsTipDoc = new ArrayList();
        try {
            ltsTipDoc = TipoDocBo.buscarNombreTipoDocumento(buscar, cargo);
            MTD.setListado_TipDoc(ltsTipDoc);
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
    ...
    private void buttonEditar() {
        ...
        int fila = this.tablaDocumento.getSelectedRow();
        if (fila != -1) {
            this.objTP = MTD.getTipDoc(fila);
            this.activarControles(true);
            this.cargarDatos(objTP);
            this.cargarTabla();
        } else {
            JOptionPane.showMessageDialog(this, "Usted no a seleccionado un campo en la tabla.", "MENSAJE --> TIPO DOCUMENTO", JOptionPane.ERROR_MESSAGE);
        }
